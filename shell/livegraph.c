...
struct _LiveGraph {
    ...
    LgRGBA *line_colors_dark;
    LgRGBA *line_colors_light;
...
static void     live_graph_init_palettes (LiveGraph *graph, gint n_signals);

GtkWidget *
live_graph_new (gint max_samples, gint n_signals)
{
    LiveGraph *graph;
    ...
    graph->n_signals = n_signals;
    graph->samples = g_malloc0 (n_signals * max_samples * sizeof (gdouble));
    graph->sample_time = g_malloc0 (max_samples * sizeof (gdouble));

    /* Populate the color palettes from the dynamic signal count. */
    live_graph_init_palettes (graph, n_signals);
...
live_graph_push (LiveGraph *graph, gint signal, gdouble value)
{
    ...
    g_return_if_fail (signal >= 0 && signal < graph->n_signals);
...
gint
live_graph_n_signals (LiveGraph *graph)
{
    g_return_val_if_fail (LIVE_IS_GRAPH (graph), 0);
    return graph->n_signals;
}
...
live_graph_select_line (LiveGraph *graph, gint signal)
{
    ...
    if (signal < 0)
        signal = -1;
    else if (signal >= graph->n_signals)
        signal = graph->n_signals - 1;
...
/* Convert an HSV color (h in degrees, s/v in [0,1]) to RGB in [0,1]. */
static void
live_graph_hsv_to_rgb (double h, double s, double v,
                       double *r, double *g, double *b)
{
    ...
    if (hp < 1.0)      { rr = c; gg = x; }
    else if (hp < 2.0) { rr = x; gg = c; }
    else if (hp < 3.0) { gg = c; bb = x; }
    else if (hp < 4.0) { gg = x; bb = c; }
    else if (hp < 5.0) { rr = x; bb = c; }
    else               { rr = c; bb = x; }
...
/* Populate the per-signal color palettes for the given number of signals.
 * Palettes are generated by cycling through a set of hue modifiers derived
 * from the signal index. */
static void
live_graph_init_palettes (LiveGraph *graph, gint n_signals)
{
    /* Base hue angles (degrees) for the dark and light palettes, cycled
     * across signals so any count produces distinct, readable colors. */
    static const double dark_hues[] = {
        198.0, 32.0, 132.0, 358.0, 268.0, 175.0, 46.0, 330.0, 90.0, 40.0
    };
    static const double light_hues[] = {
        208.0, 24.0, 142.0, 4.0, 276.0, 186.0, 52.0, 336.0, 90.0, 36.0
    };
    static const double hsv_s[] = { 0.9, 1.0, 0.9, 0.95, 0.9, 0.85, 0.95, 0.9, 0.9, 0.9 };
    static const double dark_v[] = { 0.95, 0.9, 0.9, 0.9, 0.95, 0.9, 0.9, 0.9, 0.85, 0.9 };
    static const double light_v[] = { 0.65, 0.65, 0.55, 0.6, 0.6, 0.55, 0.55, 0.6, 0.5, 0.55 };
    int i;

    g_free (graph->line_colors_dark);
    g_free (graph->line_colors_light);
    graph->line_colors_dark  = g_new0 (LgRGBA, n_signals);
    graph->line_colors_light = g_new0 (LgRGBA, n_signals);

    for (i = 0; i < n_signals; i++) {
        double h = dark_hues[i % G_N_ELEMENTS (dark_hues)];
        double s = hsv_s[i % G_N_ELEMENTS (hsv_s)];
        double v = dark_v[i % G_N_ELEMENTS (dark_v)];
        double r, g, b;
        double h2 = light_hues[i % G_N_ELEMENTS (light_hues)];
        double v2 = light_v[i % G_N_ELEMENTS (light_v)];
        double lr, lg, lb;

        live_graph_hsv_to_rgb (h, s, v, &r, &g, &b);
        lg_set_color (&graph->line_colors_dark[i], r, g, b, 1.0);

        live_graph_hsv_to_rgb (h2, s, v2, &lr, &lg, &lb);
        lg_set_color (&graph->line_colors_light[i], lr, lg, lb, 1.0);
    }
}
...
live_graph_init (LiveGraph *graph)
{
    ...
    graph->line_colors = NULL;
    graph->line_colors_dark = NULL;
    graph->line_colors_light = NULL;
